...
def submit_test(request):
    if request.method == "POST":
        ...
        for idx, mcq in enumerate(mcqs, start=1):
            user_answer = request.POST.get(f"q{idx}", "").strip().lower()
            correct_answer = mcq["correct_answer"].strip().lower()
        ...
        if correct_count == total_questions:
            praise_message = f"🔥 Excellent! You got all {total_questions} questions right. You're a {topic.title()} Pro!"
            request.session["test_report"] = {
                "analysis": praise_message,
                "study_material": None,
                "youtube_link": yt_link,
                "website_links": links,
                "topic": topic
